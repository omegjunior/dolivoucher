...
/**
 * \file    dolivoucher/admin/setup.php
 * \ingroup dolivoucher
 * \brief   DoliVoucher setup page.
 */
...
require_once '../lib/dolivoucher.lib.php';
...
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('dolivouchersetup', 'globalsetup'));
...
$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM1');
$item->fieldParams['isMandatory'] = 1;
$item->fieldAttr['placeholder'] = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';
$item->helpText = 'Tooltip text';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a Thirdparty
$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('DOLIVOUCHER_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('DOLIVOUCHER_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('DOLIVOUCHER_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('DOLIVOUCHER_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('DOLIVOUCHER_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('DOLIVOUCHER_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('DOLIVOUCHER_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf DOLIVOUCHER_MYPARAM10
$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('DOLIVOUCHER_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/dolivoucher/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=dolivoucher-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DOLIVOUCHER_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'DOLIVOUCHER_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "DoliVoucherSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-dolivoucher page-admin');
...
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "dolivoucher@dolivoucher");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DoliVoucherSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DOLIVOUCHER_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DOLIVOUCHER_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
